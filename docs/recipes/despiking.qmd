...
[`Patch.hampel_filter`](`dascore.Patch.hampel_filter`) replaces samples whose local [median absolute deviation](https://en.wikipedia.org/wiki/Median_absolute_deviation) exceeds a threshold with the local median.
...
Start with thresholds of 3.5–5 and windows of 5–9 samples per dimension (`samples=True`), then inspect the result. Use exact medians only when the approximation is insufficient.
